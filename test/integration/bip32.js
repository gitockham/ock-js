...
describe('ock-js (BIP32)', function () {
  it('can create a BIP32 wallet external address', function () {
    var path = "m/0'/0/0"
    var root = ock.HDNode.fromSeedHex('dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
  ...
  it('can create a BIP44, ock, account 0, external address', function () {
  ...
  it('can recover a BIP32 parent private key from the parent public key, and a derived, non-hardened child private key', function () {
    function recoverParent (master, child) {
      ...
      for (var i = 0; i < ock.HDNode.HIGHEST_BIT; ++i) {
        ...
        var Qp = new ock.ECPair(d2).Q
        if (Qp.equals(QP)) break
      }

      var node = new ock.HDNode(new ock.ECPair(d2), master.chainCode, master.network)
    ...
    var master = ock.HDNode.fromSeedBuffer(seed)
